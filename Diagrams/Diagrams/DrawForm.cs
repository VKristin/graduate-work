    ...
    public partial class DrawForm : Form
    {
        ...
        private void pbDraw_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (coordList.Count() != 0 && draw)
            {
                ...
                e.Graphics.DrawImage(imageList1.Images[0], startPosition.X + cellSize * position.X, startPosition.Y - 1 - cellSize * position.Y, cellSize - 2, cellSize - 2);

                for (int i = 0; i < coordList.Count(); i++)
                {
                    e.Graphics.DrawLine(pen1, new Point(coordList[i].p1.X * cellSize, (-coordList[i].p1.Y + numOfCellsY + 1) * cellSize), new Point(coordList[i].p2.X * cellSize, (-coordList[i].p2.Y + numOfCellsY + 1) * cellSize));
                }
            }
        }
